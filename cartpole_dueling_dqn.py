...
def OurModel(input_shape, action_space, dueling):
    ...
    if dueling:
        state_value = K.layers.Dense(1, kernel_initializer='he_uniform')(X)
        state_value = K.layers.Lambda(lambda s: K.backend.expand_dims(s[:, 0], -1), 
                                      output_shape=(action_space,))(state_value)
        action_advantage = K.layers.Dense(action_space, kernel_initializer='he_uniform')(X)
        action_advantage = K.layers.Lambda(lambda a: a[:, :] - K.backend.mean(a[:, :], keepdims=True), 
                                           output_shape=(action_space,))(action_advantage)
        X = K.layers.Add()([state_value, action_advantage])
    else:
    ...
    model.compile(loss="mean_squared_error", optimizer=K.optimizers.RMSprop(lr=0.00025, rho=0.95, epsilon=0.01), metrics=["accuracy"])
...
class DQNAgent:
    ...
    def PlotModel(self, score, episode):
        ...
        self.average.append(sum(self.scores[-20:]) / len(self.scores[-20:]))
